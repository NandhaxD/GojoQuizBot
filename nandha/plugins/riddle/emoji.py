...
async def check_user_remoji_ans(_, message):
...
@bot.on_callback_query(filters.regex('^remoji'))
async def riddle_emoji(_, query):
      ...
      if user_id != admin_id:
         ...
         button = [[
           InlineKeyboardButton(text=change_font('60 Sec'), callback_data=f'retime:{user_id}:60'),
           InlineKeyboardButton(text=change_font('3 Min'), callback_data=f'retime:{user_id}:90'),],
                   [InlineKeyboardButton(text=change_font('10 Min'), callback_data=f'retime:{user_id}:600'),
                    InlineKeyboardButton(text=change_font('30 Min'), callback_data=f'retime:{user_id}:1800'), ],
                   [ InlineKeyboardButton(text=change_font('Back ⬅️'), callback_data=f'cb_riddle:{user_id}')
           
]]
            
         off_button = [[
               InlineKeyboardButton(text=change_font('OFF 🛑'), callback_data=f'reoff:{user_id}'),
               InlineKeyboardButton(text=change_font('Back ⬅️'), callback_data=f'cb_riddle:{user_id}')
 ]]
         if riddle:
               return await query.message.edit(
                     text=change_font("This chat has already set up the timing for sending emoji riddles. To change the settings, turn them off and try again."),
...
@bot.on_callback_query(filters.regex('^retime'))
async def set_riddle_chat_time(_, query):
       ...
       if user_id != admin_id:
           ...
           time = await get_chat_sleep(chat_id, mode, type) 
           button = [[
                   InlineKeyboardButton(change_font('BACK ⬅️'), callback_data=f'remoji:{user_id}')

           ]]
           return await query.message.edit(
                  text=change_font(f"Successfully set up chat emoji riddle!\n\nRiddle: `Enabled` 📢\nRiddle time: `{time}` ⏰"),
                   reply_markup=InlineKeyboardMarkup(button)
           )
...
@bot.on_callback_query(filters.regex('^reoff'))
...
async def send_emoji_riddle_tochat(chat_id: int):  
        
       lock = asyncio.Lock()
       async with lock:
           while True:                                 
               sleep_time = int(await get_chat_sleep(chat_id, mode, type))
               photo, text = await make_emoji_riddle(chat_id)   
...
async def sends_emoji_riddle(_, message):
      chat_id = message.chat.id
      if not chat_id in chats_id:
            riddle = await is_chat(chat_id, mode, type)
            if riddle:
                  await clear_chat_data(chat_id, mode, type)
                  chats_id[chat_id] = asyncio.create_task(send_emoji_riddle_tochat(chat_id))
